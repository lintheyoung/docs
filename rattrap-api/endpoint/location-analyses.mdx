...
// 使用 analysis.annotated_image_url 直接加载标注图
// 或使用 analysis.annotated_media_asset_id 进行后续关联
```
</RequestExample>
...
```json 成功响应
{
  ...
  "setup_session_id": "ss_1Qy8u8CZ7aQp98Xb5WJtR3",
  "media_asset_id": "ma_room_1PpTqQx2YwZv9kLb",
  "annotated_image_url": "https://vwinvkxxheuexvpvzibt.supabase.co/storage/v1/object/public/media-assets/ma_room_annotated_9DzXcP4v",
...
<CardGroup cols={3}>
  ...
  <Card title="标注图片" icon="image">
    在原图上标出推荐区域 (`annotated_image_url` 和 `annotated_media_asset_id`)
  </Card>
...
<Steps>
  ...
  <Step title="获取标注图片">
    ...
    // 直接使用返回的 URL
    document.getElementById('annotated-image').src = analysis.annotated_image_url;
