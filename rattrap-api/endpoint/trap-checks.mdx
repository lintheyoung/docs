...
curl https://vwinvkxxheuexvpvzibt.supabase.co/functions/v1/trap-checks \
  ...
  -d '{
    "trap_id": "3c175d6d-d5af-4070-b73a-c9a5ab8e37e7",
    "media_asset_id": "a8f72b3e-9c41-4f2a-b5d3-7e1a6d8c9f0b",
    "check_type": "empty"
  }'
```
</RequestExample>
...
```json 成功响应 - 捕获分析
{
  ...
  "blurred_image_url": "https://vwinvkxxheuexvpvzibt.supabase.co/storage/v1/object/public/media-assets/ma_blurred_xyz789",
...
```json 成功响应 - 空陷阱诊断（被盗食）
{
  ...
  "annotated_image_url": "https://vwinvkxxheuexvpvzibt.supabase.co/storage/v1/object/public/media-assets/ma_annotated_abc123",
...
```json 成功响应 - 空陷阱诊断（空响）
{
  ...
  "annotated_image_url": "https://vwinvkxxheuexvpvzibt.supabase.co/storage/v1/object/public/media-assets/ma_annotated_def456",
...
```json 成功响应 - 空陷阱诊断（虎口脱险）
{
  ...
  "annotated_image_url": "https://vwinvkxxheuexvpvzibt.supabase.co/storage/v1/object/public/media-assets/ma_annotated_ghi789",
...
<ParamField body="media_asset_id" type="string" required>
  已上传的照片 ID（**UUID 格式**），使用 `POST /media-assets` 获取。必须确保照片已上传完成（status 为 `ready`）。
</ParamField>
...
<ResponseField name="blurred_image_url" type="string | null">
  模糊处理后的照片 URL。如果 `should_blur=true`，后端会自动生成模糊版本并返回可直接访问的存储 URL。

  <Note>
  URL 格式：`https://vwinvkxxheuexvpvzibt.supabase.co/storage/v1/object/public/media-assets/{id}`

  可直接用于前端 `<img>` 标签或其他需要图片 URL 的场景。
  </Note>
</ResponseField>

<ResponseField name="blurred_media_asset_id" type="string | null">
  模糊图片的媒体资源 ID。与 `blurred_image_url` 对应，可用于后续关联（如记录到 `trap_events.media_id`）。

  <Warning>
  **AI 生成图片的过期时长**：模糊图片会在 media_assets 表中创建记录，过期时长由全局配置决定（默认 30 天）。过期后图片会被自动清理。
  </Warning>
</ResponseField>
...
<ResponseField name="annotated_image_url" type="string | null">
  标注图 URL。如果 AI 检测到问题点（如陷阱倾斜、诱饵位置不佳），会在原图上标注并返回可直接访问的存储 URL。

  <Note>
  URL 格式：`https://vwinvkxxheuexvpvzibt.supabase.co/storage/v1/object/public/media-assets/{id}`

  可直接用于前端 `<img>` 标签显示标注后的图片。
  </Note>
</ResponseField>

<ResponseField name="annotated_media_asset_id" type="string | null">
  标注图片的媒体资源 ID。与 `annotated_image_url` 对应，可用于后续关联。

  <Warning>
  **AI 生成图片的过期时长**：标注图片会在 media_assets 表中创建记录，过期时长由全局配置决定（默认 30 天）。过期后图片会被自动清理。
  </Warning>
</ResponseField>
...
<Steps>
  ...
  <Step title="拍照上传">
    ...
    if (img.width < 800 || img.height < 800) {
      alert('照片分辨率过低，请重新拍摄（最低 800x800）');
      return;
    }
    ...
    <Info>
    **前端验证建议**：在上传前检查照片分辨率（至少 800x800），避免用户等待 AI 分析后才发现质量不足。
    </Info>
  </Step>
  ...
  <Step title="展示 AI 结果">
    **捕获流程**：
    ```typescript
    // 如果需要模糊，直接使用返回的 URL
    if (result.should_blur && result.blurred_image_url) {
      setImageUrl(result.blurred_image_url);  // 显示模糊版本
    }
    ```
    ...
    **空陷阱流程**：
    ...
    if (result.annotated_image_url) {
      setImageUrl(result.annotated_image_url);  // 显示标注版本
    }
    ```
  </Step>
  ...
  <Step title="记录事件（用户确认后）">
    ...
    if (result.check_type === 'catch') {
      // 捕获事件
      eventType = 'catch';
      details = {
        ai_identified_species: result.rodent_identification.species,
        ai_identified_subspecies: result.rodent_identification.subspecies,
        ai_confidence: result.rodent_identification.species_confidence,
        ai_sex: result.rodent_identification.sex,
        ai_estimated_weight_grams: result.rodent_identification.estimated_weight_grams,
        should_blur: result.should_blur,
        blur_reason: result.blur_reason
      };
    } else {
      // 检查/空触发事件
      if (result.diagnosis.scenario === 'escape') {
        eventType = 'miss';  // 虎口脱险记录为空触发
      }
      details = {
        ai_diagnosis: result.diagnosis.scenario,
        ai_confidence: result.diagnosis.scenario_confidence,
        bait_status: result.diagnosis.bait_status,
        trigger_status: result.diagnosis.trigger_status,
    ...
    <Info>
    **check_type 与 event_type 的映射关系**：
    - `check_type="catch"` → `event_type="catch"`（捕获事件）
    - `check_type="empty"` + `scenario="escape"` → `event_type="miss"`（空触发事件）
    - `check_type="empty"` + 其他场景 → `event_type="check"`（检查事件）

    **trap.next_check_at 更新逻辑**：后端会优先使用 `details.recommended_next_check_hours`（AI 推荐），如果没有则使用默认的 maintenance_rules 计算。

    **trap.status 更新**：`miss` 事件会将陷阱状态更新为 `triggered_empty`。
    </Info>
  </Step>
</Steps>

## check_type 与 event_type 映射规则

AI 巡检返回的 `check_type` 和诊断结果需要映射到正确的 `event_type` 来记录事件：

| check_type | 诊断场景 (scenario) | 应记录的 event_type | 说明 |
|-----------|-------------------|-------------------|------|
| `catch` | - | `catch` | 成功捕获老鼠 |
| `empty` | `neophobia` | `check` | 静默期，正常检查 |
| `empty` | `bait_stolen` | `check` | 被盗食，正常检查 |
| `empty` | `false_trigger` | `check` | 空响，正常检查 |
| `empty` | `escape` | `miss` | **虎口脱险，记录为空触发** |

<Warning>
**重要**：`escape`（虎口脱险）场景虽然陷阱是空的，但因为老鼠触发了陷阱又逃脱，应该记录为 `event_type="miss"`（空触发），而不是普通的 `check`。这样可以正确统计陷阱的 `stats_misses` 字段。
</Warning>
...
**验证时机**：这些质量要求由 AI 在分析时验证，`POST /media-assets` 接口不会进行质量检查。如果图片质量不足，AI 分析会返回 `IMAGE_QUALITY_INSUFFICIENT` 错误，前端需要提示用户重新拍摄。
</Note>
...
| 无效 UUID 格式 | 400 | `VALIDATION_ERROR` | trap_id 或 media_asset_id 不是有效的 UUID 格式 |
...
| 照片质量不足 | 400 | `IMAGE_QUALITY_INSUFFICIENT` | 照片模糊、过暗或分辨率过低（由 AI 分析时检测） |
