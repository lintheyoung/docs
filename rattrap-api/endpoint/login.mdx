...
```bash cURL
curl -X POST \
  'https://lrsppxarhxvxcflougbp.supabase.co/auth/v1/token?grant_type=password' \
...
```javascript Node.js
const res = await fetch("https://lrsppxarhxvxcflougbp.supabase.co/auth/v1/token?grant_type=password", {
  method: "POST",
  headers: {
