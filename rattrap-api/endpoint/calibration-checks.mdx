...
```json 布置不正确 (CALIB_FAIL)
{
  ...
  "setup_session_id": "ss_1Qy8u8CZ7aQp98Xb5WJtR3",
  "media_asset_id": "ma_calib_1PpTqQx2YwZv9kLb",
  "annotated_image_url": "https://vwinvkxxheuexvpvzibt.supabase.co/storage/v1/object/public/media-assets/ma_calib_annotated_7YzXcP4v",
...
```json 布置正确 (CALIB_SUCCESS)
{
  ...
  "annotated_image_url": "https://vwinvkxxheuexvpvzibt.supabase.co/storage/v1/object/public/media-assets/ma_calib_2_ok_annotated",
...
<CardGroup cols={3}>
  ...
  <Card title="标注图片" icon="image">
    在原图上标注问题点 (`annotated_image_url` 和 `annotated_media_asset_id`)
  </Card>
</CardGroup>
...
响应中同时返回：
- `annotated_image_url` - 可直接访问的存储 URL，用于前端显示
- `annotated_media_asset_id` - 媒体资源 ID，用于后续关联和记录
</Warning>
...
<ResponseField name="annotated_media_asset_id" type="string | null">
  ...
  - **有标注图片时**：返回新生成的 media asset ID，可通过存储 URL 访问
  - **无标注图片时**：返回 `null`（当 `need_annotated_image: false` 或标注生成失败时）
...
<ResponseField name="issues[].metrics" type="object">
  定量信息，结构根据 issue code 不同而变化。以下是各类问题的 metrics 示例：

  **距离问题 (`too_far_from_wall`)：**
  ```json
  {
    "distance_to_wall_cm": 8,
    "recommended_max_distance_cm": 1
  ...
  **方向问题 (`wrong_orientation`, `suboptimal_orientation`)：**
  ```json
  {
    "angle_deviation_deg": 20,
  ...
  **障碍物问题 (`obstruction_detected`)：**
  ...
  <Info>
    `metrics` 是动态结构，前端应优雅处理未知字段。建议以 `message` 和 `suggestion` 为主要展示内容。
  </Info>
</ResponseField>
...
<ParamField body="options.need_annotated_image" type="boolean" default="true">
  ...
  - 设为 `true`：AI 会在原图上绘制边界框标注问题区域，结果存储在 `annotated_media_asset_id`
  - 设为 `false`：不生成标注图片，`annotated_media_asset_id` 返回 `null`，可节省处理时间
</ParamField>
...
<Info>
  该字段是完全可选的，如果不传，AI 仅凭当前照片来做判断。传入后 AI 可以对比推荐位置和实际位置，提供更精确的反馈。
</Info>
...
<Frame>
  <img src="/images/calibration-check.jpg" alt="校准检查测试图片 - 陷阱布置示例" />
</Frame>

<Note>
该图片包含一个实际布置的捕鼠陷阱，可用于测试 AI 校准检查功能。测试时会分析陷阱位置、方向、诱饵状态等。
</Note>
