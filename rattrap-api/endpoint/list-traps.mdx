...
<RequestExample>
```bash cURL
curl "https://vwinvkxxheuexvpvzibt.supabase.co/functions/v1/traps?status=active&limit=10" \
...
const res = await fetch(
  "https://vwinvkxxheuexvpvzibt.supabase.co/functions/v1/traps?status=active&limit=10",
  {
    headers: {
...
{
  "object": "list",
  "url": "/traps?status=active&limit=10",
  "has_more": false,
  "data": [
    {
...
<ParamField query="ending_before" type="string">
  向前分页的游标（下一页第一个 trap 的 ID），用于反向分页回到上一页
</ParamField>
...
<Note>
`warning` 和 `triggered_caught` 状态由系统根据陷阱事件自动设置，用户无法手动设置这两个状态。创建陷阱时只能传 `active` 或 `inactive`。
</Note>
...
<ResponseField name="last_checked_at" type="integer | null">
  上次检查时间（Unix 时间戳），初始为 `null`
</ResponseField>

<ResponseField name="next_check_at" type="integer | null">
  下次检查时间（Unix 时间戳），由规则引擎自动计算。通常不会为 `null`，但在某些边界情况下可能为空。
</ResponseField>
...
|-----|------------|------------|------|
| 无效 status 值 | 400 | `VALIDATION_ERROR` | status 必须是有效枚举值 |
| 无效 limit 值 | 400 | `VALIDATION_ERROR` | limit 必须是 1-100 的整数 |
| 无效游标 | 400 | `INVALID_CURSOR` | starting_after 或 ending_before 指向的 ID 不存在或格式错误 |
| 未认证 | 401 | `UNAUTHORIZED` | 缺少或无效的 Authorization header |

<Note>
无效游标返回 400 而非 404，因为这是客户端参数错误，不是资源不存在。
</Note>
