...
<Info>
**重要**：校准阶段完成时，**必须**同时提交 `trap_name` 来创建陷阱（Trap）和部署事件（deployment Event）。
</Info>

### 校准完成并创建陷阱

在校准阶段提交 `calibration_data` 时，同时传入 `trap_name`：
...
<Tabs>
  ...
  <Tab title="校准阶段 → Completed">
    用于写入校准检查结果并创建陷阱。**必须**同时传入 `trap_name`：
...
<Warning>
支持幂等性：如果 Session 已经创建过 Trap（`created_trap_id` 不为空），重复请求会直接返回现有数据，不会创建新的 Trap。
</Warning>
