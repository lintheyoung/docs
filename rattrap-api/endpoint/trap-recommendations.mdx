...
<Note>
**图片上传说明 - existing_trap 模式的图片来源**

`existing_trap` 模式需要提供现有捕鼠器的照片，支持以下两种方式上传图片：

1. **通过 [media-assets 接口](/api/create-media-asset) 上传**（推荐）
2. **直接在 HTTP 请求中包含图片数据**（Base64 编码）

⚠️ **重要**: 后端不再预先准备测试图片，所有图片必须通过以上两种方式上传。
</Note>
...
#### 步骤 2: 上传测试图片（使用 media-assets 接口）
...
# 提取 upload_url 和 media_asset_id
...
# 2.3 记录 MEDIA_ID 用于后续步骤
echo "media_asset_id: $MEDIA_ID"
```

<Check>
**推荐使用 media-assets 接口** - 支持多种图片格式（JPEG/PNG/WebP），自动生成预签名 URL，安全可靠
</Check>

#### 步骤 3: 测试 existing_trap 模式（使用上传的 media_asset_id）
...
existing_trap_response = requests.post(
    ...
    json={
        "mode": "existing_trap",
        "rodent_target": "mouse",
        "media_asset_id": "your_media_asset_id"  # 使用 POST /media-assets 获取的 ID
    }
)
...
#### 步骤 3: 上传图片获取 media_asset_id

通过 [create-media-asset](/api/create-media-asset) 接口上传图片：
...
# 3.2 提取 upload_url 和 media_asset_id
UPLOAD_URL=$(echo $MEDIA_RESPONSE | jq -r '.upload_url')
MEDIA_ID=$(echo $MEDIA_RESPONSE | jq -r '.id')
