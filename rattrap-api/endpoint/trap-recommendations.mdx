---
title: '获取捕鼠器推荐'
openapi: 'POST /trap-recommendations'
---

<RequestExample>
```bash cURL - 识别现有工具
curl https://vwinvkxxheuexvpvzibt.supabase.co/functions/v1/trap-recommendations \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJhdXRoZW50aWNhdGVkIiwiZXhwIjoxNzMyNjE4MDAwLCJpYXQiOjE3MzI2MTQ0MDAsImlzcyI6Imh0dHBzOi8vdndpbnZreHhoZXVleHZwdnppYnQuc3VwYWJhc2UuY28iLCJzdWIiOiJ1c2VyLWlkLWhlcmUifQ.xxx" \
  -H "Content-Type: application/json" \
  -d '{
    "mode": "existing_trap",
    "rodent_target": "rat",
    "media_asset_id": "ma_1PpTqQx2YwZv9kLb",
    "user_location": {
      "country": "TW",
      "city": "Taipei",
      "environment": "apartment"
    },
    "preferences": {
      "avoid_killing": false,
      "has_children_or_pets": true,
      "budget_level": "medium"
    },
    "limit": 3
  }'
```

```bash cURL - 无工具推荐购买
curl https://vwinvkxxheuexvpvzibt.supabase.co/functions/v1/trap-recommendations \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJhdXRoZW50aWNhdGVkIiwiZXhwIjoxNzMyNjE4MDAwLCJpYXQiOjE3MzI2MTQ0MDAsImlzcyI6Imh0dHBzOi8vdndpbnZreHhoZXVleHZwdnppYnQuc3VwYWJhc2UuY28iLCJzdWIiOiJ1c2VyLWlkLWhlcmUifQ.xxx" \
  -H "Content-Type: application/json" \
  -d '{
    "mode": "no_trap",
    "rodent_target": "mouse",
    "user_location": {
      "country": "TW",
      "city": "Taichung"
    },
    "preferences": {
      "avoid_killing": true,
      "budget_level": "high"
    },
    "limit": 5
  }'
```
</RequestExample>

<ResponseExample>
```json 识别现有工具 - 适合使用
{
  "object": "trap_recommendation_result",
  "mode": "existing_trap",
  "rodent_target": "rat",
  "user_location": {
    "country": "TW",
    "city": "Taipei",
    "environment": "apartment"
  },
  "preferences": {
    "avoid_killing": false,
    "has_children_or_pets": true,
    "budget_level": "medium"
  },
  "existing_trap": {
    "object": "existing_trap_analysis",
    "media_asset_id": "ma_1PpTqQx2YwZv9kLb",
    "detected_type": "snap_trap",
    "is_suitable": true,
    "suitability_score": 0.92,
    "notes": "识别为大号弹簧老鼠夹，对成年的大褐家鼠效果良好。请注意放置位置，避免儿童和宠物接触。"
  },
  "recommended_traps": [
    {
      "object": "trap_product",
      "id": "trp_snap_heavy_01",
      "trap_type": "snap_trap",
      "label": "大号强力弹簧夹",
      "description": "适合厨房和车库，对大鼠击杀率高，成本低。",
      "for_rodent": "rat",
      "suitability_score": 0.95,
      "safety_level": "medium",
      "maintenance_level": "low",
      "price_band": "low",
      "recommended_reason": "与您现有工具类型一致，方便一次购买多只备用。"
    }
  ],
  "created": 1764038500
}
```

```json 无工具 - 购买推荐
{
  "object": "trap_recommendation_result",
  "mode": "no_trap",
  "rodent_target": "mouse",
  "user_location": {
    "country": "TW",
    "city": "Taichung"
  },
  "preferences": {
    "avoid_killing": true,
    "budget_level": "high"
  },
  "existing_trap": null,
  "recommended_traps": [
    {
      "object": "trap_product",
      "id": "trp_cage_humane_01",
      "trap_type": "cage_trap",
      "label": "人道捕鼠笼",
      "description": "不伤害老鼠，适合室内使用。需要将捕获的老鼠带离住宅区放生。",
      "for_rodent": "mouse",
      "suitability_score": 0.91,
      "safety_level": "high",
      "maintenance_level": "medium",
      "price_band": "medium",
      "recommended_reason": "符合您\"不杀生\"的偏好，并且适合台湾常见的小家鼠。",
      "purchase_hints": {
        "search_keywords": "捕鼠笼 人道 小号",
        "preferred_platforms": ["online_marketplace"],
        "estimated_price_range": "NT$300–600"
      }
    }
  ],
  "created": 1764038600
}
```

```json 错误 - LLM 服务失败
{
  "error": {
    "type": "api_error",
    "code": "LLM_SERVICE_ERROR",
    "message": "Failed to generate recommendations. Please try again.",
    "param": null,
    "doc_url": "https://docs.rattrap.ai/api/trap-recommendations"
  }
}
```

```json 错误 - 图片无法识别
{
  "error": {
    "type": "invalid_request_error",
    "code": "MEDIA_RECOGNITION_FAILED",
    "message": "Unable to identify trap from the provided image. Please provide a clearer photo.",
    "param": "media_asset_id",
    "doc_url": "https://docs.rattrap.ai/api/trap-recommendations"
  }
}
```

```json 错误 - Media Asset 不存在
{
  "error": {
    "type": "invalid_request_error",
    "code": "MEDIA_NOT_FOUND",
    "message": "Media asset not found or does not belong to this user",
    "param": "media_asset_id",
    "doc_url": "https://docs.rattrap.ai/api/trap-recommendations"
  }
}
```

```json 错误 - 缺少必填字段
{
  "error": {
    "type": "invalid_request_error",
    "code": "VALIDATION_ERROR",
    "message": "Missing required field: media_asset_id is required when mode=existing_trap",
    "param": "media_asset_id",
    "doc_url": "https://docs.rattrap.ai/api/trap-recommendations"
  }
}
```

```json 错误 - 无效枚举值
{
  "error": {
    "type": "invalid_request_error",
    "code": "INVALID_RODENT_TARGET",
    "message": "Invalid rodent_target. Must be one of: rat, mouse",
    "param": "rodent_target",
    "doc_url": "https://docs.rattrap.ai/api/trap-recommendations"
  }
}
```

```json 错误 - 无法推荐（rodent_target=unknown）
{
  "error": {
    "type": "invalid_request_error",
    "code": "UNKNOWN_RODENT_NOT_SUPPORTED",
    "message": "Cannot provide recommendations when rodent_target is 'unknown'. Please identify the rodent type first (rat or mouse) before requesting recommendations.",
    "param": "rodent_target",
    "doc_url": "https://docs.rattrap.ai/api/trap-recommendations"
  }
}
```

```json 错误 - 请求超时
{
  "error": {
    "type": "api_error",
    "code": "REQUEST_TIMEOUT",
    "message": "The recommendation generation took too long. Please try again.",
    "param": null,
    "doc_url": "https://docs.rattrap.ai/api/trap-recommendations"
  }
}
```

```json 错误 - LLM 余额不足
{
  "error": {
    "type": "api_error",
    "code": "LLM_QUOTA_EXCEEDED",
    "message": "LLM service quota exceeded. Please contact support or try again later.",
    "param": null,
    "doc_url": "https://docs.rattrap.ai/api/trap-recommendations"
  }
}
```
</ResponseExample>

## 使用说明

这是整个"工具策略阶段"的核心接口，根据用户情况提供智能推荐：

<Tabs>
  <Tab title="识别现有工具">
    当 `mode = "existing_trap"` 时：
    - 识别用户现有工具的类型
    - 评估是否适合当前 rodent
    - 如不适合，同时给出购买建议列表

    **必填字段：** `mode`、`rodent_target`、`media_asset_id`
  </Tab>
  <Tab title="推荐购买">
    当 `mode = "no_trap"` 时：
    - 只基于 rodent + 地区 + 偏好推荐购买方案
    - 返回适合的 trap 列表及购买提示

    **必填字段：** `mode`、`rodent_target`
  </Tab>
</Tabs>

<Warning>
**重要：rodent_target 必须明确**

- `rodent_target` 只接受 `"rat"` 或 `"mouse"`
- **不支持 `"unknown"`**：当老鼠种类未知时，LLM 无法做出有效的推荐
- 如果传入 `"unknown"`，API 会返回 `400 UNKNOWN_RODENT_NOT_SUPPORTED` 错误
- **建议流程**：先使用识别接口（如拍照上传）确定老鼠种类，再调用推荐接口
</Warning>

## 用户偏好参数

<ParamField body="preferences.avoid_killing" type="boolean">
  `true` 表示希望人道捕捉，不杀生
</ParamField>

<ParamField body="preferences.has_children_or_pets" type="boolean">
  家中有小孩/宠物 → 避免暴露式弹簧夹
</ParamField>

<ParamField body="preferences.budget_level" type="string">
  预算档位：`low` / `medium` / `high`
</ParamField>

<ParamField body="preferences.noise_sensitive" type="boolean">
  对咔哒声、电击声敏感
</ParamField>

<ParamField body="preferences.maintenance_tolerance" type="string">
  能接受的维护频率：`low` / `medium` / `high`
</ParamField>

## 陷阱类型说明

### `snap_trap` - 弹簧捕鼠夹
传统机械式捕鼠器，通过强力弹簧瞬间击杀。是最经典、最经济的捕鼠方式。

**特点：**
- ✅ 击杀效率高，一击致命
- ✅ 成本极低，可重复使用
- ✅ 不需要电源或化学品
- ⚠️ 需要正确设置诱饵和位置
- ⚠️ 对儿童和宠物有潜在危险
- ⚠️ 清理尸体可能不适合所有用户

**适用场景：** 厨房、仓库、车库等封闭空间；预算有限的用户；不介意接触鼠尸的用户

---

### `glue_board` - 粘鼠板
表面涂有强力粘胶的平板，老鼠踩上后无法脱身。

**特点：**
- ✅ 使用极简单，展开即可
- ✅ 价格便宜，一次性使用
- ✅ 可同时捕获多只小鼠
- ✅ 无需设置机关，不会误伤
- ⚠️ 老鼠会长时间挣扎，不够人道
- ⚠️ 粘性会沾染灰尘影响效果
- ⚠️ 大老鼠可能挣脱逃走

**适用场景：** 公寓走廊、储藏室、办公室；小家鼠问题；希望避免机械装置的用户

---

### `cage_trap` - 捕鼠笼/活捉笼
单向门设计的金属笼，老鼠进入后无法退出，但不会受伤。

**特点：**
- ✅ 完全人道，不杀生
- ✅ 可重复使用，环保
- ✅ 对儿童和宠物安全
- ✅ 适合捕获后放生或送养
- ⚠️ 需要定期检查
- ⚠️ 需要处理活鼠(带离放生)
- ⚠️ 价格相对较高

**适用场景：** 家中有小孩或宠物；佛教徒/素食主义者；农村环境可就地放生；需要确认鼠种的科研用途

---

### `electronic_trap` - 电击捕鼠器
使用高压电流瞬间击杀，现代化的电子捕鼠装置。

**特点：**
- ✅ 击杀迅速(2秒内)，相对人道
- ✅ 清洁卫生，不见血
- ✅ 可重复使用，长期成本低
- ✅ 部分型号有击杀指示灯
- ⚠️ 需要电池或电源
- ⚠️ 价格较高(NT$1000-3000)
- ⚠️ 需要定期清理内部

**适用场景：** 餐厅厨房、医疗场所等高卫生要求环境；预算充足的家庭用户；不想接触鼠尸的用户

---

### `bucket_trap` - 水桶陷阱
利用老鼠攀爬习性，诱导其跳入深桶或水桶中困住或溺死。

**特点：**
- ✅ 可以 DIY 制作，成本极低
- ✅ 一次可捕获多只老鼠
- ✅ 适合户外或大空间使用
- ⚠️ 需要一定的制作技巧
- ⚠️ 占用空间较大
- ⚠️ 如使用溺水方式较不人道
- ⚠️ 室内使用可能不美观

**适用场景：** 农场、仓库、工地等大空间；老鼠数量较多的情况；喜欢 DIY 的用户；预算极有限的场景

---

### `poison_bait` - 毒饵/鼠药
含有抗凝血剂或神经毒素的食物诱饵，老鼠食用后会在数天内死亡。

**特点：**
- ✅ 可大范围布置
- ✅ 不需要机械装置
- ✅ 对严重鼠患效果显著
- ⚠️ 对儿童和宠物极度危险
- ⚠️ 老鼠可能死在墙内或隐蔽处产生恶臭
- ⚠️ 不符合人道主义原则
- ⚠️ 可能影响食物链(猫头鹰、猫等捕食者二次中毒)

**适用场景：** 仓库、工厂等无人居住区域；严重鼠患且其他方法无效；**不推荐家庭使用**

<Warning>
**毒饵使用限制：**
- 🚫 禁止在有儿童或宠物的环境使用
- 🚫 禁止在食品加工区域使用
- ⚠️ 必须放置在专用毒饵站内，避免误食
- ⚠️ 必须遵守当地法规(部分地区限制销售)
</Warning>

---

### `other` - 其他类型
包括超声波驱鼠器、天然驱避剂、捕鼠犬/猫等非主流方式。

**特点：**
- 超声波驱鼠器: 效果存疑，科学证据不足
- 天然驱避剂(薄荷油、樟脑等): 只能短期驱赶，无法根除
- 生物防治(猫): 需要长期饲养，效果因猫而异

**适用场景：** 作为辅助手段配合其他陷阱使用；轻微鼠患的预防阶段

## existing_trap 分析结果

当 `mode = "existing_trap"` 时，响应中包含 `existing_trap` 对象：

<ResponseField name="existing_trap.detected_type" type="string">
  识别出的陷阱类型
</ResponseField>

<ResponseField name="existing_trap.is_suitable" type="boolean">
  是否适合当前 rodent_target + 用户环境
</ResponseField>

<ResponseField name="existing_trap.suitability_score" type="number">
  0–1 之间的评分，用于前端展示信心条
</ResponseField>

<ResponseField name="existing_trap.notes" type="string">
  给用户的解释说明
</ResponseField>

## 错误响应

| 场景 | HTTP 状态码 | Error Code | 说明 |
|-----|------------|------------|------|
| LLM 服务失败 | 503 | `LLM_SERVICE_ERROR` | AI 推荐服务暂时不可用，请稍后重试 |
| 图片无法识别 | 400 | `MEDIA_RECOGNITION_FAILED` | 无法从照片中识别捕鼠器，建议重新拍摄更清晰的照片 |
| Media Asset 不存在 | 404 | `MEDIA_NOT_FOUND` | 照片不存在或不属于当前用户 |
| 缺少必填字段 | 400 | `VALIDATION_ERROR` | 请求参数不完整 |
| 无效枚举值 | 400 | `INVALID_RODENT_TARGET` 等 | 参数值不在允许的枚举范围内 |
| 请求超时 | 504 | `REQUEST_TIMEOUT` | AI 处理超时，请简化请求或稍后重试 |

<Warning>
**前端错误处理建议：**
- `LLM_SERVICE_ERROR` / `REQUEST_TIMEOUT`: 显示"服务繁忙，请稍后重试"，提供重试按钮
- `MEDIA_RECOGNITION_FAILED`: 提示用户"照片不清晰，请重新拍摄"，返回拍照界面
- `MEDIA_NOT_FOUND`: 提示用户重新上传照片
- `VALIDATION_ERROR`: 根据 `param` 字段高亮对应的输入框
</Warning>

---

## 测试资源

### Preview 环境测试数据（推荐使用）

<Note>
**自动化测试数据 - 开箱即用**

后端已在 Preview 环境配置好测试数据，每次创建或重新打开 PR 时会自动初始化。无需手动上传图片，可以直接使用以下固定的测试资源。
</Note>

#### 1. 测试用户账号

| 字段 | 值 |
|------|-----|
| Email | `test@example.com` |
| Password | `test1234` |
| User ID | 动态生成（每个 PR 环境独立） |

#### 2. 测试 Media Asset IDs（可直接使用）

以下三个 ID 都指向后端预置的测试图片，已自动插入到 Preview 环境数据库：

- `ma_test_trap_image_001`
- `ma_test_trap_image_002`
- `ma_test_trap_image_003`

<Info>
**测试图片内容**: 老鼠夹实拍照片（lockable_binder_mouse_trap.png）
- **陷阱类型**: 弹簧夹或粘鼠板
- **适用目标**: `mouse` (小鼠)
- **存储位置**: 后端自动管理，无需手动上传
</Info>

---

### 快速测试流程（Preview 环境）

#### 步骤 1: 登录获取 JWT Token

```bash
curl -X POST "https://vwinvkxxheuexvpvzibt.supabase.co/auth/v1/token?grant_type=password" \
  -H "apikey: YOUR_ANON_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "test@example.com",
    "password": "test1234"
  }'
```

**响应示例**:
```json
{
  "access_token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
  "token_type": "bearer",
  "expires_in": 3600,
  "refresh_token": "...",
  "user": {
    "id": "dynamic-user-id",
    "email": "test@example.com"
  }
}
```

保存 `access_token` 用于后续 API 调用。

#### 步骤 2: 测试 existing_trap 模式（使用预置 media_asset_id）

```bash
curl -X POST "https://vwinvkxxheuexvpvzibt.supabase.co/functions/v1/trap-recommendations" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "mode": "existing_trap",
    "rodent_target": "mouse",
    "media_asset_id": "ma_test_trap_image_001"
  }'
```

<Check>
**无需上传图片** - 直接使用 `ma_test_trap_image_001`、`ma_test_trap_image_002` 或 `ma_test_trap_image_003` 即可测试
</Check>

#### 步骤 3: 测试 no_trap 模式

```bash
curl -X POST "https://vwinvkxxheuexvpvzibt.supabase.co/functions/v1/trap-recommendations" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "mode": "no_trap",
    "rodent_target": "rat",
    "preferences": {
      "avoid_killing": true,
      "has_children_or_pets": true,
      "budget_level": "medium"
    }
  }'
```

---

### Python 自动化测试示例（Preview 环境）

```python
import requests

# Preview 环境配置
BASE_URL = "https://vwinvkxxheuexvpvzibt.supabase.co"
ANON_KEY = "your_anon_key_here"

# 1. 登录获取 token
login_response = requests.post(
    f"{BASE_URL}/auth/v1/token?grant_type=password",
    headers={
        "apikey": ANON_KEY,
        "Content-Type": "application/json"
    },
    json={
        "email": "test@example.com",
        "password": "test1234"
    }
)

token = login_response.json()["access_token"]
print(f"✅ 登录成功，Token: {token[:20]}...")

# 2. 测试 existing_trap 模式（使用预置的 media_asset_id）
existing_trap_response = requests.post(
    f"{BASE_URL}/functions/v1/trap-recommendations",
    headers={
        "Authorization": f"Bearer {token}",
        "Content-Type": "application/json"
    },
    json={
        "mode": "existing_trap",
        "rodent_target": "mouse",
        "media_asset_id": "ma_test_trap_image_001"  # 固定的测试 ID
    }
)

result = existing_trap_response.json()
print(f"✅ 识别结果: {result['existing_trap']['detected_type']}")
print(f"✅ 适配评分: {result['existing_trap']['suitability_score']}")

# 3. 测试 no_trap 模式
no_trap_response = requests.post(
    f"{BASE_URL}/functions/v1/trap-recommendations",
    headers={
        "Authorization": f"Bearer {token}",
        "Content-Type": "application/json"
    },
    json={
        "mode": "no_trap",
        "rodent_target": "rat",
        "preferences": {
            "avoid_killing": True,
            "budget_level": "medium"
        }
    }
)

recommendations = no_trap_response.json()
print(f"✅ 推荐数量: {len(recommendations['recommended_traps'])}")
```

---

### 测试环境重点说明

<Warning>
**重要提示**

1. **自动初始化**: 每次 PR 首次部署时，测试数据会自动插入，无需手动创建
2. **固定 IDs**: 测试 media_asset_id 是固定的，可以直接写在测试脚本中
3. **独立环境**: 每个 PR 都有独立的 Preview Branch，互不干扰
4. **仅限测试**: 这些测试数据只在 Preview 环境有效，**生产环境不可用**
5. **无需上传**: 不需要手动调用 create-media-asset API 上传图片
</Warning>

#### 可用的测试 Media Asset IDs

| ID | 用途 | 状态 |
|----|------|------|
| `ma_test_trap_image_001` | existing_trap 模式测试 | ✅ 可用 |
| `ma_test_trap_image_002` | existing_trap 模式测试 | ✅ 可用 |
| `ma_test_trap_image_003` | existing_trap 模式测试 | ✅ 可用 |

---

### 本地测试图片（备用方案）

如果需要测试图片上传流程，我们也提供了测试图片文件：

<Note>
**本地测试图片信息**
- **文件名**: rat_tape.png
- **图片内容**: 粘鼠板捕鼠器实拍照片
- **文件大小**: 160 KB
- **存放位置**: `rattrap-api/test-assets/rat_tape.png`
- **陷阱类型**: `glue_board` (粘鼠板)
- **适用目标**: `rat` (大鼠)
</Note>

### 完整测试流程（含图片上传）

#### 步骤 1: 获取测试图片

从文档仓库获取测试图片：

```bash
# 方式 1: 从本地文档仓库复制
cp rattrap-api/test-assets/rat_tape.png ./test_trap_image.png

# 方式 2: 从 GitHub 下载（假设文档已发布）
curl -O https://raw.githubusercontent.com/your-org/docs/main/rattrap-api/test-assets/rat_tape.png
```

#### 步骤 2: 上传图片获取 media_asset_id

首先需要通过 [create-media-asset](/api/create-media-asset) 接口上传图片：

```bash
# 上传测试图片
curl -X POST https://vwinvkxxheuexvpvzibt.supabase.co/functions/v1/media-assets \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -F "file=@rat_tape.png" \
  -F "asset_type=trap_image"
```

响应示例：
```json
{
  "id": "ma_abc123xyz456",
  "asset_type": "trap_image",
  "storage_url": "https://...",
  "created_at": 1764403600
}
```

#### 步骤 3: 使用 media_asset_id 测试推荐接口

使用获取到的 `media_asset_id` 测试捕鼠器识别：

```bash
curl -X POST https://vwinvkxxheuexvpvzibt.supabase.co/functions/v1/trap-recommendations \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "mode": "existing_trap",
    "rodent_target": "rat",
    "media_asset_id": "ma_abc123xyz456"
  }'
```

**预期响应**：

API 应该识别出这是一个 `glue_board`（粘鼠板），并给出适配性分析和推荐：

```json
{
  "object": "trap_recommendation_result",
  "mode": "existing_trap",
  "rodent_target": "rat",
  "existing_trap": {
    "object": "existing_trap_analysis",
    "media_asset_id": "ma_abc123xyz456",
    "detected_type": "glue_board",
    "is_suitable": true,
    "suitability_score": 0.75,
    "notes": "识别为粘鼠板。对大鼠有一定效果，但建议使用更大号的粘板或配合使用弹簧夹。"
  },
  "recommended_traps": [
    {
      "object": "trap_product",
      "id": "trp_glue_large_01",
      "trap_type": "glue_board",
      "label": "加大号强力粘鼠板",
      "description": "专为大鼠设计的超大粘板，粘性更强...",
      "for_rodent": "rat",
      "suitability_score": 0.90,
      "recommended_reason": "比您现有的粘板面积更大，粘性更强，对大鼠捕获率更高"
    }
  ]
}
```

### 测试注意事项

<Warning>
**重要提示**

1. **media_asset_id 是一次性的**: 每次上传图片都会生成新的 ID，不能重复使用固定 ID
2. **测试数据隔离**: 测试环境和生产环境的 media assets 是分开的
3. **认证要求**: 测试时需要使用有效的 JWT Token
4. **图片要求**:
   - 格式：PNG, JPG, JPEG
   - 大小：最大 5 MB
   - 内容：必须包含清晰的捕鼠器图像
</Warning>

### 自动化测试示例

如果你使用 Python 进行自动化测试，可以参考以下代码：

```python
import requests

BASE_URL = "https://vwinvkxxheuexvpvzibt.supabase.co/functions/v1"
AUTH_TOKEN = "your_jwt_token_here"

# 1. 上传测试图片
with open("rattrap-api/test-assets/rat_tape.png", "rb") as f:
    upload_response = requests.post(
        f"{BASE_URL}/media-assets",
        files={"file": f},
        data={"asset_type": "trap_image"},
        headers={"Authorization": f"Bearer {AUTH_TOKEN}"}
    )

media_asset_id = upload_response.json()["id"]
print(f"✅ 上传成功，media_asset_id: {media_asset_id}")

# 2. 测试捕鼠器识别
recommendation_response = requests.post(
    f"{BASE_URL}/trap-recommendations",
    json={
        "mode": "existing_trap",
        "rodent_target": "rat",
        "media_asset_id": media_asset_id
    },
    headers={
        "Authorization": f"Bearer {AUTH_TOKEN}",
        "Content-Type": "application/json"
    }
)

result = recommendation_response.json()
print(f"✅ 识别结果：{result['existing_trap']['detected_type']}")
print(f"✅ 适配评分：{result['existing_trap']['suitability_score']}")
```

### 测试图片预览

测试图片内容如下（粘鼠板实拍）：

![测试用粘鼠板图片](../test-assets/rat_tape.png)

**图片说明**：
- 捕鼠器类型：粘鼠板 (glue_board)
- 拍摄角度：俯视 45 度角
- 背景：室内地面
- 用途：测试 existing_trap 模式的图片识别功能
