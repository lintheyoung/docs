...
为了更可靠地检测设备在线状态，建议在 Shadow 文档中包含 `connected` 属性。

<Note>
**适用范围**：
- **网关设备**：可以使用完整的MQTT LWT机制配合`connected`属性
- **子设备**：由于没有直接的MQTT连接，只能由网关维护其`connected`属性，无法使用MQTT LWT机制
</Note>
...
- MQTT LWT 消息在网关异常断开时由 AWS IoT 自动发布
...
网关连接时，向非保留主题注册 LWT 消息，将 `connected` 设置为 `false`：
...
网关连接成功后，立即更新 Shadow 的 `connected` 属性为 `true`：
...
网关正常断开前，应主动更新 `connected` 为 `false`：
...
```
网关启动 → 注册 LWT (connected: false) → 连接成功 → 更新 Shadow (connected: true)
                                                              ↓
                                                     正常运行、数据上报
                                                              ↓
                                    ┌─────────────────────────┴─────────────────────────┐
...
网关可以使用完整的MQTT LWT机制，因为它拥有直接的MQTT连接：
...
<Warning>
子设备**无法使用MQTT LWT机制**，因为：
1. 子设备没有自己的MQTT连接（通过BLE/Zigbee连接网关）
2. 子设备没有AWS IoT证书
3. MQTT LWT只能在MQTT连接时设置

因此，子设备的`connected`属性必须由网关根据本地连接状态（BLE/Zigbee）来维护和更新。
</Warning>

```python
# 子设备 Shadow 更新 Topic（网关代理）
shadow_topic = f"$aws/things/{gateway_thing_name}/shadow/name/{shadow_name}/update"

# 子设备通过本地通信（BLE/Zigbee）连接到网关时
def on_subdevice_connected(gateway, shadow_name):
    """子设备连接到网关（本地BLE/Zigbee连接建立）"""
...
def on_subdevice_disconnected(gateway, shadow_name):
    """子设备从网关断开（本地BLE/Zigbee连接断开）"""
    shadow_topic = f"$aws/things/{gateway.thing_name}/shadow/name/{shadow_name}/update"
    gateway.mqtt_client.publish(shadow_topic, json.dumps({
        "state": {"reported": {"connected": False}}
    }), qos=1)

# 网关异常断开时，子设备的connected状态处理
def on_gateway_disconnected(gateway):
    """网关异常断开时，所有子设备的connected状态由后端IoT Rule处理"""
    # 网关的LWT会触发，后端可以通过IoT Rule批量更新所有子设备状态为offline
    pass
```
...
def get_device_online_status(thing_name, shadow_name=None):
    ...
    注意：
    - 网关：connected 表示与AWS IoT的MQTT连接状态
    - 子设备：connected 表示与网关的本地连接状态（BLE/Zigbee）
    """
    if shadow_name:
        # 子设备 Named Shadow
...
- **子设备**：**不能使用MQTT LWT机制**
  - 子设备没有直接的MQTT连接，无法设置LWT
  - 子设备的`connected`属性必须由网关维护
  - `connected`属性表示子设备与网关的本地连接状态（BLE/Zigbee），非与云端的连接状态
  - 当网关异常断开时，需要通过IoT Rule批量将该网关下所有子设备状态更新为offline

- **心跳机制配合**：`connected`属性应与心跳机制配合使用，提供双重保障
</Warning>
