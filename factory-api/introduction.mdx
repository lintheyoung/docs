...
Factory API 专为设备制造商设计，提供批量设备创建、激活码管理等功能。支持创建**网关设备**和**子设备**（传感器、捕鼠器等）。所有接口均需要**管理员权限**。
...
<CardGroup cols={2}>
  <Card title="网关设备" icon="router">
    中心网关，管理所有子设备，支持WiFi/4G连接
  </Card>

  <Card title="智能捕鼠器" icon="trap">
    智能捕鼠装置，通过网关连接云端
  </Card>

  <Card title="震动检测传感器" icon="wave-pulse">
    检测震动和移动，实时上报异常
  </Card>

  <Card title="温湿度传感器" icon="temperature-half">
    监测环境温湿度变化
  </Card>

  <Card title="红外人体传感器" icon="person-walking">
    人体红外感应，检测人员进入
  </Card>
</CardGroup>
...
```javascript
const gatewayResponse = await fetch('/manufacturing-create-devices', {
  method: 'POST',
  headers: {
  ...
  body: JSON.stringify({
    ...
    count: 10,
    prefix: 'GATEWAY',
    batch_name: 'Gateway Batch 2024-Q1'
  })
});

const { batch_id, devices } = await gatewayResponse.json();
console.log(`Created ${devices.length} gateways in batch ${batch_id}`);
// 记录第一个网关的Thing名称，用于创建子设备
const firstGateway = devices[0].thing_name; // "GATEWAY:001"
```

### 2. 创建子设备（智能捕鼠器）

<Warning>
**重要**: 必须先创建网关设备，再创建子设备。子设备需要指定所属的网关。
</Warning>

```javascript
const trapResponse = await fetch('/manufacturing-create-devices', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${adminToken}`,
    'Content-Type': 'application/json'
  ...
  body: JSON.stringify({
    ...
    count: 50,
    prefix: 'TRAP',
    gateway_thing_name: 'GATEWAY:001',  // 指定所属网关
    batch_name: 'Smart Trap Batch 2024-Q1'
  })
});

const trapData = await trapResponse.json();
console.log(`Created ${trapData.created_count} smart traps for GATEWAY:001`);
```
...
<Steps>
  <Step title="1. 创建网关设备">
    首先批量创建网关设备，指定 `device_type: "gateway"`
  </Step>

  <Step title="2. 记录网关Thing名称">
    保存每个网关的Thing名称（如 GATEWAY:001），用于创建子设备
  </Step>

  <Step title="3. 创建子设备">
    为每个网关创建子设备，指定 `gateway_thing_name` 参数
  </Step>
  ...
  <Step title="7. 设备配对出厂">
    网关和子设备配对打包，状态为 `registered`，等待用户绑定
  </Step>
</Steps>
...
```
┌──────────────────┐
│  GATEWAY:001     │ ← 网关设备（WiFi/4G连接云端）
└────────┬─────────┘
         │
         ├─► TRAP:001 (智能捕鼠器)
         ├─► TRAP:002 (智能捕鼠器)
         ├─► VIBSENSOR:001 (震动传感器)
         ├─► TEMPSENSOR:001 (温湿度传感器)
         └─► PIRSENSOR:001 (红外传感器)
              ↑
         子设备通过BLE/Zigbee/LoRa连接网关
```

<Note>
**设备关联**: 子设备在创建时通过 `gateway_thing_name` 字段与网关关联。用户绑定子设备时，其所属的网关必须已经绑定。
</Note>
