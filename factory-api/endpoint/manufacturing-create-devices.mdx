...
<ResponseField name="devices" type="array">
  ...
  <Expandable title="Device 对象">
    <ResponseField name="thing_name" type="string">
      AWS IoT Thing名称（仅网关设备），格式: `{prefix}-{6位数字编号}`

      例如: `IoT-Gateway-000001`

      子设备此字段为 `null`
    </ResponseField>

    <ResponseField name="shadow_name" type="string">
      Named Shadow名称（仅子设备），格式: `{prefix}-{6位数字编号}`

      例如: `vibration-sensor-001`
    ...
    <ResponseField name="name" type="string">
      ...
      - 网关设备: 与 `thing_name` 相同
      - 子设备: 与 `shadow_name` 相同
    </ResponseField>
...
<Note>
**子设备响应说明**：
- 子设备不需要 AWS IoT 凭证（通过网关连接），因此证书相关字段返回空字符串
- `thing_name` 为 `null`，`shadow_name` 和 `name` 使用 Named Shadow 名称
- 网关设备的 `shadow_name` 为 `null`，`thing_name` 和 `name` 使用 Thing 名称
</Note>
...
| **震动检测传感器** | `vibration_sensor` | 检测震动和移动 | 通过网关 | 1-100 台 |
| **智能捕鼠器** | `smart_trap` | 智能捕鼠装置 | 通过网关 | 1-100 台 |
| **温湿度传感器** | `temp_humidity_sensor` | 监测环境温湿度 | 通过网关 | 1-100 台 |
| **红外人体传感器** | `pir_sensor` | 人体红外感应 | 通过网关 | 1-100 台 |

<Note>
**设备类型说明**：
- **网关设备**：生成完整的 AWS IoT 凭证（Thing、证书、私钥）
- **子设备**：仅生成激活码，不创建 AWS IoT 资源（通过网关连接云端）
- 所有设备类型均使用本接口创建，批次限制 1-100 台
</Note>
...
<Note>
**接口特性**:
- 本接口用于创建所有设备类型（网关设备和子设备）
- 网关设备生成完整的 AWS IoT 凭证（Thing、证书、私钥）
- 子设备仅生成激活码，不创建 AWS IoT 资源
- 批次限制：1-100 台设备
</Note>
