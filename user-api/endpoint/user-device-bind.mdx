---
title: '绑定设备'
openapi: 'POST /user-device-bind'
---

## 接口说明

用户通过激活码绑定设备，建立设备所有权关系。该接口支持绑定网关设备和子设备，是设备从制造到使用流程中的关键步骤。
...
- **网关设备**：4G/WiFi 网关，直接连接云端，管理多个子设备
- **子设备**：通过网关间接连接云端，使用 AWS IoT Named Shadows
- **通信协议**：子设备通过 BLE/Zigbee/LoRa 与网关通信
...
| `tenant_id` | `null` | 用户的租户ID |
| `parent_id` | `null` | 网关ID（仅子设备） |
| `name` | 默认名称 | 用户昵称（如提供） |
| `updated_at` | 创建时间 | 绑定时间 |

### device_activation_codes 表
...
| `used_at` | `null` | 绑定时间 |
| `used_by` | `null` | 用户UUID |
| `bound_gateway_id` | `null` | 网关ID（仅子设备） |
...
1. **激活码一次性使用**：激活码绑定后立即标记为已使用，防止重复绑定
2. **设备所有权唯一性**：一个设备只能属于一个用户
3. **网关归属验证**：绑定子设备时严格验证网关是否属于当前用户
4. **租户隔离**：通过 RLS 策略确保用户只能访问自己租户的数据
...
<Steps>
  ...
  <Step title="2. 输入子设备激活码">
    用户手动输入子设备（捕鼠器、传感器等）包装上的激活码
  </Step>
